...
(function (jQuery) {
    ...
    validation.wordSimilarToUsername = function (options, word, score) {
      ...
      if (
        ...
        word
          .toLowerCase()
          .match(
            username.replace(/[-[\]/{}()*+=?:.\\^$|!,]/g, "\\$&").toLowerCase()
          )
      ) {
        return score;
      }
        ...
        $.each(rulesEngine.forbiddenSequences, function (idx, seq) {
          ...
          $.each(sequences, function (ignore, sequence) {
            for (j = 0; j < word.length - 2; j += 1) {
              ...
              if (
                sequence.indexOf(word.toLowerCase().substring(j, j + 3)) > -1
              ) {
                found = true;
              }
            }
  ...
  (function ($) {
    ...
    ui.initProgressBar = function (options, $el) {
      ...
      if (options.ui.bootstrap2) {
        // Boostrap 2
        progressbar += options.ui.progressBarExtraCssClasses + '"><div class="';
      } else {
        // Bootstrap 3 & 4
        progressbar +=
  ...
  (function () {
    ...
    ui.updatePopover = function (options, $el, verdictText, remove) {
      ...
      if (
        ...
        html =
          '<h5><span class="password-verdict">' + verdictText + "</span></h5>";
        hide = false;
      }
      if (options.ui.showErrors) {
  ...
  (function ($) {
    ...
    onKeyUp = function (event) {
      ...
      if (word.length === 0) {
        ...
        if (options.common.zxcvbn) {
          userInputs = [];
          $.each(
            options.common.userInputs.concat([options.common.usernameField]),
    ...
    $.fn.pwstrength = function (method) {
      ...
      if (methods[method]) {
        ...
        $.error(
          "Method " + method + " does not exist on jQuery.pwstrength-bootstrap"
        );
      }

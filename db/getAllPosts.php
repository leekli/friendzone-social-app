<?php
    ...
    $statement = "SELECT * FROM posts ORDER BY date_posted DESC;";
    ...
    if(!$result){
        ...
        echo "<center>";
        echo "<h4>📄 All posts</h4><small><em>(Ordered by date posted)</em></small><br /><br />";
        while($row = mysqli_fetch_assoc($result)) {
            ...
            session_start();
            if ($_SESSION['id'] == session_id() && $current_user == $row['username']) {   
                echo '<form action="db/deletePost.php?post_id=' . $row['post_id'] . '" method="POST">';
                echo '<center><button type="submit" class="btn btn-danger btn-sm" id="deletePostButton">Delete Post</button></center>';
                echo '</form>';   
                echo '<br />';
            }
